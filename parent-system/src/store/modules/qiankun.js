...
const mutations = {
  ...
  setTabItems(state, data) {
    ...
    setTimeout(() => {
      globalStore.setGlobalState({
        type: "redirectMainRoute",
        tabList: JSON.parse(JSON.stringify(state.tabItems))
      });
    }, 100);
  },
  ...
  delTabs(state, index) {
    ...
    (index == len || isActive) &&
      router.push({ path: state.tabItems[state.tabItems.length - 1]["path"] });
